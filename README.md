...
### 3b. Setup Storage (untuk foto bukti retur)
Skema tabel ada di migrations, tapi bucket Storage dibuat lewat console Supabase (di luar SQL). Di Supabase Dashboard > Storage:
1. Buat bucket baru bernama `return-photos`, set **Public**.
2. Tambah 2 policy di `storage.objects` untuk bucket itu:
   - **INSERT** untuk role `authenticated` (hanya user login yang boleh upload).
   - **SELECT** untuk `public` (foto dibuka lewat URL ber-UUID acak; tidak ada policy UPDATE/DELETE, jadi foto tidak bisa ditimpa/dihapus lewat API).

### 3c. Buat user admin pertama
Sistem hanya punya 1 role (Admin). Di Supabase Dashboard > Authentication > Users, buat user baru (email + password). Profil-nya (dengan role `admin`) dibuat otomatis oleh trigger `fn_handle_new_user` saat user pertama kali ada. Pakai email + password itu untuk login ke aplikasi.
