...
| 0129 | `fix_bundle_recipe_unique_constraint.sql` | **Bugfix drift**: unique constraint lama `(bundle_product_id, component_product_id)` di `bundle_recipes` gak pernah tercatat di migration manapun, dan gak ikut diperbarui waktu `0112` nambah versioning -- akibatnya edit resep yang MASIH pakai komponen dari versi sebelumnya gagal disimpan ("duplicate key"). Diganti jadi partial unique index yang cuma berlaku untuk baris `is_active = true` |
| 0130 | `block_duplicate_product_name.sql` | Ketemu saat QA testing: SKU sudah unik sejak awal, tapi NAMA produk belum dijaga sama sekali. Tambah unique index case-insensitive + trim (`lower(trim(name))`) di `products`, biar "Cushion"/"cushion"/" Cushion " dianggap duplikat yang sama |
...
- `bundle_recipes`: 1 komponen gak boleh dobel dalam 1 versi resep yang aktif, ditegakkan lewat partial unique index `WHERE is_active = true` (`0129`, ganti dari constraint lama yang salah scope, drift gak tercatat sebelumnya).
